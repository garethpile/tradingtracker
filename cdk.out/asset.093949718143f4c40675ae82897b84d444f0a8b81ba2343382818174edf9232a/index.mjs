...
var SECRET_PLACEHOLDER = "__SET_ME__";
...
var getAllowedChatIds = (secret) => {
  ...
  if (Array.isArray(raw)) {
    return new Set(
      raw.map((entry) => String(entry).trim()).filter((entry) => entry.length > 0 && entry !== SECRET_PLACEHOLDER)
    );
  }
  return new Set(
    String(raw ?? "").split(",").map((entry) => entry.trim()).filter((entry) => entry.length > 0 && entry !== SECRET_PLACEHOLDER)
  );
};
var getOptionalSecretValue = (value) => {
