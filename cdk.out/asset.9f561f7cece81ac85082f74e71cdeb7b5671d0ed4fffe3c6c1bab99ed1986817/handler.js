...
const parseStructuredTelegramTrade = (text) => {
  ...
  const takeProfitApproach = fieldMap.get('TAKE_PROFIT_APPROACH');
...
const buildStructuredSummary = (parsedTrade) => {
  const summaryLines = [
    parsedTrade.tradingAsset ? `SYMBOL: ${parsedTrade.tradingAsset}` : null,
    parsedTrade.tradeSide ? `POSITION: ${parsedTrade.tradeSide.toUpperCase()}` : null,
    parsedTrade.entryPrice !== undefined ? `ENTRY: ${parsedTrade.entryPrice}` : null,
    parsedTrade.exitPrice !== undefined ? `EXIT: ${parsedTrade.exitPrice}` : null,
    parsedTrade.lotSize !== undefined ? `LOT: ${parsedTrade.lotSize}` : null,
    parsedTrade.strategy ? `METHOD: ${parsedTrade.strategy}` : null,
    parsedTrade.stage ? `STAGE: ${parsedTrade.stage}` : null,
  ].filter(Boolean);

  if (parsedTrade.confluences.length > 0) {
    summaryLines.push(`CONFLUENCES: ${parsedTrade.confluences.join(', ')}`);
  }

  return summaryLines.join('\n');
};

const buildStructuredComments = (parsedTrade, fallbackCaption) => {
  const commentLines = [];

  if (parsedTrade.stage) {
    commentLines.push(`Stage: ${parsedTrade.stage}`);
  }
  if (parsedTrade.takeProfitApproach) {
    commentLines.push(`Take profit approach: ${parsedTrade.takeProfitApproach}`);
  }
  if (parsedTrade.comments.length > 0) {
    commentLines.push(...parsedTrade.comments);
  }
  if (parsedTrade.invalidatedConfluences.length > 0) {
    commentLines.push(`Invalidated confluences: ${parsedTrade.invalidatedConfluences.join('; ')}`);
  }
  if (commentLines.length === 0 && fallbackCaption) {
    commentLines.push(fallbackCaption);
  }
...
export const handler = async (event) => {
  ...
  try {
    ...
    if (parsedTrade) {
      ...
      extractedSummary = buildStructuredSummary(parsedTrade);
      ...
      payload = {
        ...
        feelings: 'Not filled',
        comments: buildStructuredComments(parsedTrade, caption),
